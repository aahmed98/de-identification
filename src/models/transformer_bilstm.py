...
def transformer(hparams, name="transformer"):
  ...
  outputs = tf.keras.layers.Dense(
      units=hparams.d_model, name="outputs",activation="relu")(enc_outputs)
...
class TransformerBiLSTM(tf.keras.Model):
    ...
    def call(self,inputs):
        ...
        """
        output = self.transformer_full.call(inputs = [inputs]) # (batch_size, max_len, rnn_size) 
        outputs = self.bi_lstm(output) # (batch_size, max_len, 2*rnn_size)
        outputs = self.d1(outputs) # (batch_size, max_len, tag_size)
        return outputs
